...
def get_last_month_version_downloads_per_version(session: Session, package: str) -> list[tuple[str, int]]:
    last_month = datetime.date.today() - datetime.timedelta(days=30)

    # get date available in database
    date_query = (
        session.query(CondaDownload.date)
        .filter(CondaDownload.pypi_name == package, CondaDownload.date >= last_month)
        .group_by(CondaDownload.date)
        .order_by(CondaDownload.date)
        .all()
    )
    ...
    month_begin_download = dict(
        session.query(CondaDownload.version, func.sum(CondaDownload.download_count))
        .filter(CondaDownload.pypi_name == package, CondaDownload.date == min_date)
        .group_by(CondaDownload.version)
        .all()
    )
    month_end_download = dict(
        session.query(CondaDownload.version, func.sum(CondaDownload.download_count))
        .filter(CondaDownload.pypi_name == package, CondaDownload.date == max_date)
        .group_by(CondaDownload.version)
        .all()
    )
    return [(version, month_end_download.get(version, 0) - month_begin_download.get(version, 0)) for version in month_end_download.keys()]
